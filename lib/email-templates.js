...
// Org Invite
// ─────────────────────────────────────────────
export function orgInvite({ inviterName, inviteeName, orgName, role, inviteUrl }) {
  const subject = `You've been invited to join ${orgName} on CostDeck`;
  const greeting = inviteeName ? `Hi ${inviteeName},` : 'Hi there,';
  const html = base(subject, `
    ${h1("You've been invited to join an organization")}
    ${p(`${greeting}<br><br><strong>${inviterName}</strong> has invited you to join <strong>${orgName}</strong> on CostDeck as a <strong>${role}</strong>.`)}
    ${divider()}
    <table cellpadding="0" cellspacing="0" style="margin-bottom:28px;">
