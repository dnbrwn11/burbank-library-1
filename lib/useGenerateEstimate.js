...
export function useGenerateEstimate() {
  ...
  const readStream = useCallback(async (res, onBatch, onBatchError) => {
    ...
    return { globals, ai_assumptions, failed };
  }, []);
    ...
    try {
      ...
      const { globals, ai_assumptions, failed } = await readStream(
        ...
        (newItems) => {
          ...
          // Sort by sort_order so display order is correct even if chunks arrive out of order
  ...
  const retryChunk = useCallback(async (chunkIndex, description, project) => {
    setIsGenerating(true);
    ...
    try {
      ...
      const { failed } = await readStream(
        ...
        (newItems) => {
          ...
          // Merge into the existing items, replacing sort_order range for this chunk
          setItems(prev => {
